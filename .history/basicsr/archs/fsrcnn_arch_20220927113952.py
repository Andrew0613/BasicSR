...
class FSRCNN(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, d=56,s=12,m=4, upscale_factor=3,init_type="kaiming"):
        ...
        layers = []
        feature_extraction_layer = [nn.Conv2d(in_channels, d, kernel_size=5, padding=0),nn.PReLU(d)] # 11->7
        layers.extend(feature_extraction_layer)
        shrinking_layer = [nn.Conv2d(d, s, kernel_size=1, padding=0),nn.PReLU(s)]#7->7
        layers.extend(shrinking_layer)
        mapping_layer = []
        for _ in range(m):
            mapping_layer.append(nn.Conv2d(s, s, kernel_size=3, padding=1))#7->7
            mapping_layer.append(nn.PReLU(s))
        layers.extend(mapping_layer)
        expanding_layer = [nn.Conv2d(s, d, kernel_size=1, padding=0),nn.PReLU(d)]#7->7
        layers.extend(expanding_layer)
        deconv_layer = [nn.ConvTranspose2d(d, out_channels, kernel_size=9, stride=upscale_factor, padding=4, output_padding=0),nn.PReLU(out_channels)]#7->19
