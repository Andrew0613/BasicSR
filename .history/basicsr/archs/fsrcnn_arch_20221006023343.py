...
class FSRCNN(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, d=56,s=12,m=4, upscale=3,is_residual = False,init_type="kaiming",no_modified=False):
        ...
        mid_layers = []
        shrinking_layer = [nn.Conv2d(d, s, kernel_size=1, padding=0),nn.PReLU(s)] if no_modified else None
        mid_layers.extend(shrinking_layer)
        for _ in range(m):
            mid_layers.append(nn.Conv2d(s, s, kernel_size=3, padding=1))
            mid_layers.append(nn.PReLU(s))
        expanding_layer = [nn.Conv2d(s, d, kernel_size=1, padding=0),nn.PReLU(d)] if no_modified else None
