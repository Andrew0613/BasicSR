...
class FSRCNN(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, d=56,s=12,m=4, upscale_factor=3,init_type="kaiming"):
        ...
        mid_part.append(self.shrinking_layer)

        for _ in range(m):
            mid_part.append(nn.Conv2d(s, s, kernel_size=3, padding=1))
            mid_part.append(nn.PReLU(s))
        self.mapping_layer = nn.Sequential(*mid_part)
