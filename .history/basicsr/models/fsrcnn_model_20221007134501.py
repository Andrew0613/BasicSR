...
class FSRCNNModel(SRModel):
    ...
    def setup_fsrcnn_optimizers(self):
        ...
        if is_finetune:
            ...
            a = self.net_g.module.extraction_layer.parameters()
            self.optimizer_g = optim.SGD([
                {'params': self.net_g.module.extraction_layer.parameters()},
                {'params': self.net_g.module.mid_layers.parameters()},
                {'params': self.net_g.module.deconv_layer.parameters(), 'lr': lr * 0.1}
            ], lr=lr,momentum=mom, weight_decay=wd)
