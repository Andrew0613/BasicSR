...
def generate_train_img(dataset_path,output_path,up_scale=3,patch_size=11):
    """
    生成训练集
    ...
    for img_path in tqdm(img_path_list):
        ...
        for up_scale in range(6,10):
            for roatation in [0,90,180,270]:
                new_img = img.resize((img.size[0]*up_scale//10,img.size[1]*up_scale//10),Image.BICUBIC)
    ...
    for hr in tqdm(img_list):
            hr_width = (hr.width // up_scale) * up_scale
            hr_height = (hr.height // up_scale) * up_scale
            hr = hr.resize((hr_width, hr_height), resample=Image.BICUBIC)
            lr = hr.resize((hr.width // up_scale, hr_height // up_scale), resample=Image.BICUBIC)
            hr = hr.convert('YCbCr')
            lr = lr.convert('YCbCr')
            hr = np.array(hr).astype(np.float32)
            lr = np.array(lr).astype(np.float32)
            for i in range(0, lr.shape[0] - patch_size + 1, up_scale):
                for j in range(0, lr.shape[1] - patch_size + 1, up_scale):
                    lr_patches.append(lr[i:i+patch_size, j:j+patch_size])
                    hr_patches.append(hr[i*up_scale:i*up_scale+patch_size*up_scale, j*up_scale:j*up_scale+patch_size*up_scale])
    #保存数据集
