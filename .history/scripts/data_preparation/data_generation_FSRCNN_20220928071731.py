...
def generate_train_img(dataset_path,output_path,scale=3):
    """
    生成训练集
    ...
    for hr in img_list:
            ...
            for i in range(0, lr.shape[0] - args.patch_size + 1, scale):
                for j in range(0, lr.shape[1] - args.patch_size + 1, scale):
                    lr_patches.append(lr[i:i+args.patch_size, j:j+args.patch_size])
                    hr_patches.append(hr[i*scale:i*scale+args.patch_size*scale, j*scale:j*scale+args.patch_size*scale])
