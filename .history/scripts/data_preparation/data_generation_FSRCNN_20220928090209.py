...
def generate_train_img(dataset_path,output_path,scale=3,patch_size=11):
    ...
    #删除文件夹
    if os.path.exists(os.join(output_path,'lr%s'%(scale))):
        os.remove(os.join(output_path,'lr%s'%(scale)))
    if os.path.exists(os.join(output_path,'hr%s'%(scale))):
        os.remove(os.join(output_path,'hr%s'%(scale)))
    if not os.path.exists(os.join(output_path,'lr%s'%(scale))):
        os.makedirs(os.join(output_path,'lr%s'%(scale)))
    if not os.path.exists(os.join(output_path,'hr%s'%(scale))):
        os.makedirs(os.join(output_path,'hr%s'%(scale)))
