...
def generate_train_img(dataset_paths:list,output_path,up_scale=3,patch_size=11,is_residual=False,phase="train",interpolation="bicubic"):
    """
    生成训练集
    ...
    """
    # 获取数据集路径
    interpolate = Image.BICUBIC if interpolation == "bicubic" else Image.BILINEAR
    ...
    for label in tqdm(img_list):
            ...
            label = label.resize((label_width, label_height), resample=interpolate)
            data = label.resize((label.width // up_scale, label_height // up_scale), resample=interpolate)
            unscale = data.resize((data.width * up_scale, data.height * up_scale), resample=interpolate)
    ...
    # h5_file.close()
    #这种方式保存太过于耗时
    data_name = "data_x{}_residual".format(up_scale) if is_residual else "data_x{}".format(up_scale) +interpolation
    label_name = "label_x{}_residual".format(up_scale) if is_residual else "label_x{}".format(up_scale) +interpolation
    #删除文件夹
